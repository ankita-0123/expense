  ...
  return async (dispatch) => {
    const fecthData = async () => {
        const res = await axios.get(
          `https://expense-37e43-default-rtdb.firebaseio.com/${email}.json`
        );
      ...
      const editExpense = async () => {
        try {
          const res = await axios.put(
            `https://expense-37e43-default-rtdb.firebaseio.com/Expense${email}/${id}.json`, obj
          );
    ...
    const removeExpense = async () => {
      try {
        const res = await axios.delete(
          `https://expense-37e43-default-rtdb.firebaseio.com/Expense${email}/${id}.json`
        );
      } catch (error) {
        console.log(error);
      }
